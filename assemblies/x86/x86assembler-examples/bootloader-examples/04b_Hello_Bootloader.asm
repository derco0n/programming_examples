...
initcolor:
mov bx, 0 ; Zur Sicherheit das komplette BX-Register mit 0x00 initialisieren.
mov bl, 0 ; bl müsste nun schon den Wert 0x0 haben, dennoch den Farbwert zur Sicherheit nochmal setzen

printstr:
; Iteriert Zeichen für Zeichen durch den String

; Prüfen ob Farbwert grösser 254 ist und fall ja, die Farbe auf null zurücksetzen. (Max. 255 Farben in diesem Modus)
; https://de.wikibooks.org/wiki/Assembler-Programmierung_f%C3%BCr_x86-Prozessoren/_Spr%C3%BCnge_und_Schleifen
cmp bx,0xfe ; 254
ja initcolor ; jump if above	CF = 0 und ZF = 0	Operand1 > Operand2
...
inc bx ; EBX hoch interieren um Farben zu wechseln
...
; Diese Funktion stellt eine Endlosschleife dar in welcher nichts passiert
rerun:
...
sometext db "Dies ist ein langer Text um die Farbiterationen anhand des BX-Registers zu verdeutlichen. Bei jedem Zeichen wird die Vordergrundfarbe und nach jeweils 16 Zeichen die Hintergrundfarbe iteriert. Nach 128 Zeichen wird Bit 7 gesetzt (Blink-Flag) (Beim 'd' des Wortes 'Vordergrundfarbe'). Nach 256 (0xFF) (beim Zeichen ' ' vorm Wort 'Wortes') wird die Farbnummer geresetted.... (Dieser Text ist 409 Zeichen lang.)", 0x00

; Dieser Teil...
times 510 - ($-$$) db 0 ; Dieses Programm auf bis zu 510 Bytes mit Nullen auffüllen um die Größe von 512 Bytes zu erreichen
dw 0xAA55 ; Die standard PC boot signatur ( und die fehlende 2 Bytes um auf 512 zu kommen)
